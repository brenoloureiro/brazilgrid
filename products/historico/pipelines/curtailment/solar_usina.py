"""
Daily Pipeline - restricao_coff_fotovoltaica_detail_tm
Pipeline AUTO-CORRETIVO que processa todos os meses pendentes desde a última data

Lógica:
1. Busca última data no ClickHouse
2. Identifica meses incompletos (última_data → hoje)
3. Processa cada mês incrementalmente
...
S3_PREFIX = "dataset/restricao_coff_fotovoltaica_detail_tm/"
TABLE = "curtailment_solar_usina"
DATABASE = "brazilgrid_historico"
RAW_DATA_DIR = Path("data/raw")
MANIFEST_PATH = Path("data/processed") / f"{DATASET}_manifest.json"

# Data início dos dados (Abr/2024)
DATA_START = datetime(2024, 4, 1)
...
@task(name="Download arquivo S3 Solar", retries=2, retry_delay_seconds=5, cache_policy=NONE)
def download_s3_task(s3_handler: S3Handler, s3_key: str, local_path: Path) -> str:
    """
    Download do arquivo Parquet do S3 ONS
    Sempre redownload para pegar atualizações
    """
    logger = get_logger()

    # Sempre baixar arquivo (pode ter atualizações)
...
@task(name="Inserir no ClickHouse Solar", retries=2, retry_delay_seconds=5, cache_policy=NONE)
def insert_clickhouse_task(
    ...
    try:
        # Buscar última data no ClickHouse para este arquivo
        max_date_query = f"""
        ...
        if max_date:
            logger.info(f"Última data no CH: {max_date}")
        else:
            logger.info(f"Primeira inserção na tabela")
...
@task(name="Processar mês solar", cache_policy=NONE)
def process_month_task(
    ...
    """
    Processa um mês (download + insert incremental)
    """
    logger = get_logger()
    ...
    s3_key = f"{S3_PREFIX}RESTRICAO_COFF_FOTOVOLTAICA_DETAIL_{year}_{month}.parquet"
    local_filename = f"RESTRICAO_COFF_FOTOVOLTAICA_DETAIL_{year}_{month}_{download_date}.parquet"
...
def get_months_to_process(last_date_in_db: datetime, today: datetime) -> List[str]:
    ...
    Args:
        last_date_in_db: Última data com dados no ClickHouse
        today: Data atual

    Returns:
        Lista de meses no formato ['2024-04', '2024-05']
    """
    months = []

    # Se não tem dados ou data muito antiga, começar do DATA_START
    if last_date_in_db is None or last_date_in_db.year < 2020:
        current = DATA_START
    else:
        # Começar do mês da última data
...
@flow(
    name="Daily Update AUTO-CORRETIVO - restricao_coff_fotovoltaica_detail_tm",
    description="Processa TODOS os meses pendentes desde última data até hoje",
    log_prints=True
)
def daily_restricao_solar_usina():
    """
    Pipeline diário AUTO-CORRETIVO
    ...
    # Header
    logger.info("="*80)
    logger.info("DAILY UPDATE AUTO-CORRETIVO - restricao_coff_fotovoltaica_detail_tm")
    logger.info("="*80)
    ...
    try:
        ...
        logger.info("Verificando se ClickHouse está acordado...")
        if wake_clickhouse():
    ...
    # Buscar última data no ClickHouse
    try:
        ...
        if last_date:
            logger.info(f"Última data no ClickHouse: {last_date}")

            # Calcular dias de atraso
            days_behind = (today - last_date).days
            logger.info(f"Atraso: {days_behind} dias")
        else:
            logger.info(f"Tabela vazia - primeira execução")
            last_date = None

    except Exception as e:
        logger.error(f"Erro ao buscar última data: {e}")
        raise
    ...
    try:
        ...
        logger.info(f"Período: {stats[1]} → {stats[2]}")
...
if __name__ == "__main__":
    ...
    python products/historico/pipelines/curtailment/solar_usina.py
    """
    daily_restricao_solar_usina()
