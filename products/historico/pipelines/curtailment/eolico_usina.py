"""
Daily Pipeline - restricao_coff_eolica_detail_tm
...
S3_PREFIX = "dataset/restricao_coff_eolica_detail_tm/"
...
# Data início dos dados (Out/2023)
...
@task(name="Processar mês", cache_policy=NONE)
def process_month_task(
    ...
    s3_key = f"{S3_PREFIX}RESTRICAO_COFF_EOLICA_DETAIL_{year}_{month}.parquet"
    local_filename = f"RESTRICAO_COFF_EOLICA_DETAIL_{year}_{month}_{download_date}.parquet"
...
@flow(
    name="Daily Update AUTO-CORRETIVO - restricao_coff_eolica_detail_tm",
    description="Processa TODOS os meses pendentes desde última data até hoje",
    log_prints=True
)
def daily_restricao_usina():
    """
    ...
    # Header
    logger.info("="*80)
    logger.info("DAILY UPDATE AUTO-CORRETIVO - restricao_coff_eolica_detail_tm")
    logger.info("="*80)
    ...
    try:
        ...
        if wake_clickhouse():
            ...
            logger.warning("⚠️ ClickHouse pode estar lento para acordar")
...
if __name__ == "__main__":
    ...
    python products/historico/pipelines/curtailment/eolico_usina.py
    """
    daily_restricao_usina()
